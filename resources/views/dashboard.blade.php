...
<div class="flex justify-center">
	<div class="w-full md:w-8/12 lg:w-6/12 flex flex-col items-center md:flex-row">
		<div class="w-8/12 lg:w-6/12 px-5">
			<img class="rounded-full" src="{{ $user->imagen ? asset('perfiles') . '/' . $user->imagen : asset('img/user.png') }}" alt="" />
		</div>
		<div class="md:w-8/12 lg:w-6/12 px-5 flex flex-col items-center md:justify-center md:items-start py-10 md:py-10">
			...
			<form action="{{ route('users.follow', $user) }}" method="POST" accept-charset="utf-8">
				@csrf
			...
			@else
			<form action="{{ route('users.unfollow', $user) }}" method="POST" accept-charset="utf-8">
				@method('DELETE')
				@csrf
